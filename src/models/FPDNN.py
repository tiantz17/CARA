...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.CompoundEncoding = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(p=0.2),
            nn.Linear(self.encode2, self.encode3),
            nn.BatchNorm1d(self.encode3),
            nn.ReLU(),
            nn.Dropout(p=0.2),
            nn.Linear(self.encode3, 1),
        )
        self.apply(weights_init)

    def forward(self, input, label):
        comp = input[0]
        affinity = self.CompoundEncoding(comp)
