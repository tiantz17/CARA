...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.ProteinEncoding = nn.Sequential(
            nn.Embedding(26, self.seq_input_dim), 
            Transpose(1, 2),
            nn.Conv1d(self.seq_input_dim, self.hidden, self.seq_kernal_size, padding=int(np.ceil((self.seq_kernal_size-1)/2))),
            nn.ReLU(),
            nn.Conv1d(self.hidden, self.hidden*2, self.seq_kernal_size, padding=int(np.ceil((self.seq_kernal_size-1)/2))),
            nn.ReLU(),
            nn.Conv1d(self.hidden*2, self.hidden*3, self.seq_kernal_size, padding=int(np.ceil((self.seq_kernal_size-1)/2))),
            nn.ReLU(),
            Max(-1),
        )
        ...
        self.Output = nn.Sequential(
            nn.Linear(self.hidden*3*2, self.output1),
            nn.ReLU(),
            nn.Dropout(p=0.1),
    ...
    def forward(self, input, label):
        ...
        pa = self.ProteinEncoding(protein_a)
