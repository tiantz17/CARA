...
from src.datasets.DeepConvDTI import DeepConvDTIData
...
class DataSet(CARADatasetMeta, DeepConvDTIData):
    """
...
#         print("support : query,", len(x_spt[0]), len(x_qry[0]))
        x_spt[0] = torch.Tensor(x_spt[0])
        x_qry[0] = torch.Tensor(x_qry[0])
        y_spt['Affinity'] = torch.Tensor(y_spt['Affinity'])
        y_qry['Affinity'] = torch.Tensor(y_qry['Affinity'])
        
#         list_time.append(time.time())
#         print(sample_idx, len(support_x), len(query_x), list_time[1] - list_time[0], list_time[2] - list_time[1], list_time[3] - list_time[2])
